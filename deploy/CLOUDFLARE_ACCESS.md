...
不要在 `deploy/.env` 中再放一份生产密码；实际密码均从 `deploy/secrets/` 读取。
...
```bash
sudo docker compose --project-directory ./deploy --env-file ./deploy/.env \
  -f ./deploy/docker-compose.yml -f ./deploy/docker-compose.cloudflare.yml \
  logs --tail=100 db-init migrate
sudo docker compose --project-directory ./deploy --env-file ./deploy/.env \
  -f ./deploy/docker-compose.yml -f ./deploy/docker-compose.cloudflare.yml \
  logs --tail=100 api web caddy cloudflared
```
...
sudo docker compose --project-directory ./deploy --env-file ./deploy/.env \
  -f ./deploy/docker-compose.yml -f ./deploy/docker-compose.cloudflare.yml pull
sudo docker compose --project-directory ./deploy --env-file ./deploy/.env \
  -f ./deploy/docker-compose.yml -f ./deploy/docker-compose.cloudflare.yml up -d --no-build
sudo docker compose --project-directory ./deploy --env-file ./deploy/.env \
  -f ./deploy/docker-compose.yml -f ./deploy/docker-compose.cloudflare.yml ps
```
...
sudo docker compose --project-directory ./deploy --env-file ./deploy/.env \
  -f ./deploy/docker-compose.yml -f ./deploy/docker-compose.cloudflare.yml \
  logs --tail=200 cloudflared caddy
```
...
- `deploy/.env` 中 `ADMIN_DOMAIN` 与 Tunnel hostname 完全一致；
- `ADMIN_ALLOWED_CIDRS=172.30.0.40/32`；
